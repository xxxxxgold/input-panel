...
<img src="src/assets/project-logo.webp" alt="Input Panel Logo" width="112" />
...
| `.\restart-frontend-backend.cmd` | 可见日志窗口与显式重启 | 替换已有受控实例, 分别打开前后端常驻窗口 |
...
> [!IMPORTANT]
> `start.cmd` 以 `src-tauri/target/debug/app.exe` 为启动目标. 首次使用前请先运行 `pnpm build:desktop:debug`.
